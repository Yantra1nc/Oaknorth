...
define(['N/record', 'N/search', 'N/runtime', 'N/email','N/format','N/file','N/task','N/sftp','N/xml','N/config'],
	...
	function getInputData(context){
		...
		try{
			...
			log.debug('getInputData',' subsidiaryId  --> '+subsidiaryId);
			...
			log.debug('getInputData','DomainId=='+DomainId);
			log.debug('getInputData','clientCode=='+clientCode);
	...
	function summarize(summary) {   
		try{ 	
				...
				summary.output.iterator().each(function(key, value)
					...
					/*	var customrecord_hdfc_bank_detailsSearchObj = search.create({
							...
							filters:
								[
									...
									columns:
										[
											search.createColumn({
												name: "custbody_vendor_name_oaknorth",
						...
						customrecord_hdfc_bank_detailsSearchObj.run().each(function(result){
							...
							html += '<tr>'
								html += '<td style="border-right:1px solid black; border-bottom:0px solid black">'+result.getValue({name: "custbody_vendor_name_oaknorth", join: "CUSTRECORD_HBD_TRANSACTION", label: "Vendor Name"})+'<\/td>'
					...
					vendorbillSearchObj.run().each(function(result){
						...
						if(attachmentLinkDocId){
							...
							var netsuiteUrl = 'https://5123585.app.netsuite.com'
								var  finalUrl =  netsuiteUrl + to_userlogo.url 
			...
			email.send({
				author:userId,
				//author:-5,
				//recipients:'gopal.garg@oaknorth.co.uk,puneet.chaddha@oaknorth.co.uk,accountspayable.platform@oaknorth.com,pralhad@yantrainc.com',
				recipients:'accountspayable.platform@oaknorth.com',
				subject: 'Vendor Bill Payment File',
				body:'Below payments are successfully uploaded in HDFC Bank. <br/> <br/>'+html,
